...
export class GdmLiveAudioVisuals3D extends LitElement {
  ...
  private _outputNode!: AudioNode | null;
  ...
  private animation() {
    ...
    if (sphereMaterial.userData.shader) {
      ...
      const outputData1 = this.outputAnalyser?.data[1] ?? 0;
      const outputData2 = this.outputAnalyser?.data[2] ?? 0;
      const outputData0 = this.outputAnalyser?.data[0] ?? 0;


      // Smoothly update the sphere's scale
      const targetScale = 1 + (0.2 * outputData1) / 255;
