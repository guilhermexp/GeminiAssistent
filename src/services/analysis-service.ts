...
export class AnalysisService {
  ...
  private async generateTagsForSummary(summary: string): Promise<string[]> {
    try {
      const prompt = `Baseado no seguinte resumo, gere de 3 a 5 tags semânticas relevantes, de uma única palavra, em minúsculas. As tags devem descrever o tópico principal ou a categoria do conteúdo (ex: código, design, dados, tutorial, marketing). Retorne APENAS uma lista separada por vírgulas, sem texto adicional. Resumo: "${summary}"`;
      ...
      console.error('Falha ao gerar tags:', error);
...
${content}`;
    ...
    const tags = await this.generateTagsForSummary(summary);
    return {
      summary,
      title: contentTitle,
...
`;
  ...
  private async analyzeGoogleSheetUrl(
    ...
    const contentTitle = `Planilha Google - ${sheetKey}`;
...
${csvData}`;
  ...
  private async analyzeGenericUrl(
    ...
    const logMsg = url.includes('docs.google.com/document/')
      ...
      : `Analisando URL`;
    setProcessingState(true, 'Analisando URL com IA...', 50);
    logEvent(logMsg, 'process');

    const analysisPrompt = `Você é um especialista em análise de conteúdo da web. Sua tarefa é visitar a seguinte URL, analisar seu conteúdo completamente e criar um resumo detalhado que servirá como sua base de conhecimento.
URL para analisar: ${url}

Seu resumo deve ser formatado em markdown e incluir:
1.  **Título da Página**: O título principal da página.
2.  **Resumo Detalhado**: Um resumo completo dos pontos principais, argumentos e conclusões do conteúdo.
3.  **Pontos-Chave**: Uma lista dos insights mais importantes em formato de bullet points.

Prepare-se para responder a perguntas sobre o conteúdo. Lembre-se, sua resposta deve ser inteiramente em português do Brasil.`;
    ...
    const summary = response.text;
    if (!summary?.trim()) {
      throw new Error('A análise da URL retornou um resultado vazio.');
    }

    // We'll use the URL as the title for simplicity, as the summary will contain the real title.
    const contentTitle = url;
    const tags = await this.generateTagsForSummary(summary);
